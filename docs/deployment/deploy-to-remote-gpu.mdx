...
For example, set the browser origin if you will open the dashboard through a Brev public URL, raise the post-create readiness budget when sandbox initialization is slow, and then run the installer:
...
- Resource-constrained cloud VMs where the created sandbox takes longer to initialize.
...
If onboard ends with `Sandbox '<name>' was created but did not become ready within 180s`, it prints a lifecycle receipt.
