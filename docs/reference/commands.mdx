...
For each registered OpenClaw or Hermes sandbox, `doctor` also sends a read-only `probe` request to the managed lifecycle controller.
The probe does not restart or recover the gateway or change its configuration.
A status-0 response with exactly one `GATEWAY_PID=<positive integer>` completion marker and no recognized failure marker reports `Managed lifecycle: ok`.
If no controller result is available, or a failed controller request returns `SUPERVISOR_BUSY` without another recognized failure marker, the check reports a warning.
A lifecycle, security, or controller refusal, an ambiguous diagnostic result, or a response without a trusted completion marker reports a failure.
Failure details can include only one allowlisted marker and, where applicable, one allowlisted control stage.
NemoClaw does not surface raw controller output, appended exception text, paths, or environment data.
...
For gateway runtimes, `doctor` also reports an informational `Serving process: not checked` result.
The `Managed lifecycle` check can prove supervisor and gateway health, but the fresh sandbox inference probe does not attest the long-running gateway process's inference access.
This result does not fail the readiness check.

</AgentOnly>
<AgentOnly variant="deepagents">

Terminal runtimes omit the `Serving process` check because they have no long-running gateway process.
...
This includes unavailable probe transport or controller contention.
Failed checks, including a failed `Managed lifecycle` check or a failed or unavailable authoritative inference route, exit non-zero so scripts can use `doctor` as a readiness gate.
