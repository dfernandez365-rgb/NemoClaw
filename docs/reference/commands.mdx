...
For the bundled Hermes quiet-chat path, a completed turn exits with status `0`, an interrupted turn exits with status `130`, and every other incomplete result exits non-zero.
When Hermes exhausts its iteration budget in quiet mode, it prints a deterministic `Incomplete:` result instead of requesting another model-generated summary, so verify the requested artifacts before retrying.
