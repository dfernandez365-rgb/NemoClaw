...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | `180` | Wall-clock timeout for the readiness wait that starts after sandbox creation, in seconds. Image build and gateway upload finish before this budget starts. Raise when in-sandbox boot or readiness checks exceed the default. When the deadline expires, onboarding emits a lifecycle receipt. Retry only when the CLI prints a retry hint. Read `terminal_resolution` and follow the emitted cleanup guidance when the CLI omits the hint. |
...
The readiness wait emits a lifecycle receipt.
Read `terminal_resolution` before you retry:

- `timed_out_deleted` or `terminal_failure_deleted`: Cleanup deleted the sandbox or confirmed it was absent. The CLI prints a retry hint.
- `timed_out_retained` or `terminal_failure_retained`: Deletion failed. The CLI treats the sandbox as retained and prints manual cleanup guidance.
- `deferred_to_docker_gpu_patch`: The GPU compatibility path owns cleanup. The CLI does not print a retry hint.

Do not retry when the CLI omits the retry hint.
