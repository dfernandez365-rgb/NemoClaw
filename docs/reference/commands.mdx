...
On ordinary routes, successful deletion or an already-absent result permits the retry hint; failed deletion retains the sandbox and prints manual cleanup guidance.
Compatibility routes defer cleanup to the GPU patch and do not advertise an immediate retry.
