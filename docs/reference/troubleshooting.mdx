...
When the deadline expires, NemoClaw prints a lifecycle receipt and attempts to delete the exact sandbox on ordinary routes.
The retry hint appears only when deletion succeeds or the sandbox is already absent.
Failed deletion retains the sandbox and prints manual cleanup guidance; compatibility routes defer cleanup to the GPU patch and do not advertise an immediate retry.
