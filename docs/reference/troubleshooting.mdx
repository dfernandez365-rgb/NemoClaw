...
```text
  ...
  Deleted sandbox 'my-assistant' after the readiness gate failed; retry can recreate it.
...
It does not cover the inference probe, image build, or gateway upload.
The 180-second default fits typical workstations but can be exceeded when:

- The created sandbox takes longer to initialize on a resource-constrained host.
- The selected model is large (70B+ parameters or 4-bit/8-bit quantisations that take time to memory-map).
- A provider and its egress policy stack add work during in-sandbox boot.
...
When the deadline expires, NemoClaw prints a lifecycle receipt.
