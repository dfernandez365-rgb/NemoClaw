...
- The managed DGX Spark Qwen3.6 profile now leaves MTP speculative decoding disabled while retaining async scheduling and its other registered serve arguments.
  The vLLM setup guide also documents an optional profile that bounds context, concurrency, and batch size while disabling async scheduling.
  Physical DGX Spark testing did not reproduce a complete host freeze, so these profiles do not establish freeze prevention.
