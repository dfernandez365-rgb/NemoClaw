...
For `nvidia/Qwen3.6-35B-A3B-NVFP4`, the managed DGX Spark default keeps `--async-scheduling` and does not enable MTP speculative decoding.
The default retains the registered context, concurrency, batching, quantization, cache, parser, and load-format arguments.
...
For the managed Qwen3.6 profile on DGX Spark, use this optional profile when containment is more important than throughput.
...
This profile disables async scheduling, limits each request to `32,768` tokens, permits one active sequence, and limits each batch to `4,096` tokens.
It intentionally reduces concurrency and throughput.
The physical DGX Spark matrix did not reproduce a complete host freeze, so neither profile proves that a freeze cannot occur.
