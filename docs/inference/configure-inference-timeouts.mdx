...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | Post-create readiness polling, including in-sandbox boot | `180` seconds |
...
Raise `NEMOCLAW_SANDBOX_READY_TIMEOUT` when onboarding creates the sandbox but its post-create boot or readiness checks exceed 180 seconds.
Image build and gateway upload finish before this budget starts, so slow build or upload increases total onboarding time without consuming the readiness timeout.
This can occur when a large model, a larger policy stack, or constrained host resources delay sandbox initialization.
